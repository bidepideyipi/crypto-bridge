...
// TestAddressTransactions 额外测试：获取地址交易列表
func (s *ChainTestSuite) TestGetAddressTransactions() {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()

	s.T().Run("获取活跃地址的交易", func(t *testing.T) {
		// 使用一个可能有交易的测试网地址
		address := "tb1q5pshmp3yq6kl9q2hpyxqmn8ypdxk2c3jv2k7a8"

		txs, err := s.btcAdapter.GetTransactions(ctx, address, "")
		if err != nil {
			s.T().Logf("获取交易列表失败（地址可能无交易）: %v", err)
			return
		}

		s.T().Logf("地址 %s 的交易数量: %d", address, len(txs))

		// 验证交易结构
		for i, tx := range txs {
			s.NotEmpty(tx.TxID, fmt.Sprintf("交易 %d 的 ID 不应为空", i))
			s.Greater(tx.Version, 0, fmt.Sprintf("交易 %d 的版本应该大于 0", i))

			if tx.Status.Confirmed {
				s.Greater(tx.Status.BlockHeight, 0, fmt.Sprintf("交易 %d 的区块高度应该大于 0", i))
			}

			// 只检查前几个交易的详细信息
			if i < 3 {
				s.T().Logf("交易 %d: ID=%s, 手续费=%d, 已确认=%v",
					i, tx.TxID, tx.Fee, tx.Status.Confirmed)
			}
		}
	})

	s.T().Run("分页获取交易", func(t *testing.T) {
		address := "tb1q5pshmp3yq6kl9q2hpyxqmn8ypdxk2c3jv2k7a8"

		// 获取第一批交易
		firstBatch, err := s.btcAdapter.GetTransactions(ctx, address, "")
		if err != nil {
			s.T().Skip("无法获取第一批交易")
			return
		}
		...
		lastTxID := firstBatch[len(firstBatch)-1].TxID

		// 使用最后交易的 ID 获取下一批
		secondBatch, err := s.btcAdapter.GetTransactions(ctx, address, lastTxID)
		if err != nil {
			s.T().Logf("获取第二批交易失败: %v", err)
			return
		}

		s.T().Logf("第一批交易数: %d, 第二批交易数: %d", len(firstBatch), len(secondBatch))

		// 验证没有重复
		if len(secondBatch) > 0 {
			s.NotEqual(secondBatch[0].TxID, firstBatch[len(firstBatch)-1].TxID,
				"分页获取的交易不应该重复")
		}
	})
}

// TestMempoolTransactions 测试内存池交易获取
func (s *ChainTestSuite) TestMempoolTransactions() {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()

	address := "tb1q5pshmp3yq6kl9q2hpyxqmn8ypdxk2c3jv2k7a8"

	txs, err := s.btcAdapter.GetMempoolTransactions(ctx, address)
	if err != nil {
		s.T().Logf("获取内存池交易失败: %v", err)
		return
	}

	s.T().Logf("地址 %s 的内存池交易数量: %d", address, len(txs))

	// 验证内存池交易的特性
	for _, tx := range txs {
		s.False(tx.Status.Confirmed, "内存池交易不应被确认")
		s.Equal(0, tx.Status.BlockHeight, "内存池交易的区块高度应为 0")
	}
}

// TestChainType 测试链类型获取
func (s *ChainTestSuite) TestChainType() {
	chainType := s.btcAdapter.GetChainType()
	s.Equal("BTC", chainType, "链类型应该是 BTC")
}

// TestAdapterClose 测试适配器关闭
func (s *ChainTestSuite) TestAdapterClose() {
	// 创建一个新的适配器来测试关闭
	testAdapter := btc.NewAdapter(
		[]string{"https://blockstream.info/testnet/api"},
		"testnet",
		5*time.Second,
		1,
		s.logger,
	...
	s.NotNil(testAdapter, "应该能创建适配器")

	// 关闭适配器应该不报错
	err := testAdapter.Close()
	s.NoError(err, "关闭适配器应该成功")
}
